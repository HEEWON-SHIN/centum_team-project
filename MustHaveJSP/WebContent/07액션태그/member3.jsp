...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    %>	
    	...
    	<%-- 위에서 useBean액션태그로 생성한 MemberBean객체 내부의 setter메소드를 호출할 때
    		 입력한 아이디를 매개변수로 전달하여 MemberBean객체의 인스턴스 변수에 저장시키자
    		 
    		 name속성값 : useBean액션태그로 작성한 MemberBean객체의 참조변수명
    		 
    		 property속성값 : MemberBean객체의 인스턴스 변수명
    		 
    		 value속성값 : MemberBean객체의 인스턴스 변수에 저장할 값(memberForm3.html에서 입력받은 값,,요청한 값 얻어서 저장)
    		 
    		 
    		 	참고로! <jsp:useBean>액션태그를 사용해서 자바빈 객체를 생성해놔야지만
    		 			<jsp:setProperty>액션태그를 사용할 수 있다!!!
    		 			(반대로 자바코드를 이용해서 자바빈을 생성해놓은 경우에는
    		 
    	 --%>
    
    
    	<jsp:setProperty  name="mBean" property="id" value='<%=request.getParameter("id") %>'/><%--표현식 넣을 때는 안쪽이 큰따옴표, 바깥이 작은따옴표!! --%>
    	<jsp:setProperty  name="mBean" property="pwd" value='<%=request.getParameter("pwd") %>'/>
    	<jsp:setProperty  name="mBean" property="name" value='<%=request.getParameter("name") %>'/>
    	<jsp:setProperty  name="mBean" property="email" value='<%=request.getParameter("email") %>'/>
